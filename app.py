...
@st.cache_resource
def init_connection():
    return pymongo.MongoClient(os.getenv('URI'))
...
def load_data():
    ...
    try:
        # Index Name
        index_name = "earning-calls-euclidean"
        index = PineconeVectorStore(index_name=index_name, embedding=embeddings, pinecone_api_key=os.getenv('PINECONE_API_KEY'))
        ...
        with st.spinner(text="Thinking ..."):
            with st.chat_message("assistant"):
                ...
                try:
                    ...
                    response_full = st.session_state.rag_pipeline.invoke(prompt)
